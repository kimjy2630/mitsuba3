...
class AS {
public:
    static std::vector<int> pivots;
    static const unsigned int arr_N = 5;
    int a[arr_N];

    constexpr static int ii(int a) {
        return a * 2;
    }

    //    constexpr static const std::vector<int> initPivots(int N) {
     ...
     * p = c1 * 4^(n-1) + ... + cn * 4^0
     *
     * { 4^0, 4^0 + 4^1, ..., 4^0 + 4^1 + ... + 4^N } pre-computed array가 존재
     * binary search를 이용하여 p >= 4^0 + ... + 4^(n-1)인 max n을 탐색
     * p1 := p - (4^0 + ... + 4^(n-1))
     * p1을 4진수 형태로 나타내면
     * p1 = b1 * 4^(n-1) + ... + bn * 4^0
     * ck = bk + 1 for k = 1...n
     ...
     * @return
     */
    std::vector<unsigned int> num_to_index(unsigned int p) {
        std::vector<unsigned int> ret;

        std::lower_bound(pivots.begin(), pivots.end(), p);
        std::upper_bound(pivots.begin(), pivots.end(), p);


    }
};

class LinearAS : public AS {

};

class QTAS : public AS {

};

constexpr unsigned int aa(const unsigned int n) {
    if (n < 1)
        return 0;
    if (n == 1)
        return 1;
...
constexpr unsigned int x1 = AS::ii(3);
constexpr unsigned int x2 = aa(15);

//std::vector<int> AS::pivots = std::vector<int>();
std::vector<int> AS::pivots = { aa(1), aa(2) };
//std::vector<int> AS::pivots = AS::initPivots(3);
...
class AdaTestIntegrator : public MonteCarloIntegrator<Float, Spectrum> {
public:
    MI_IMPORT_BASE(MonteCarloIntegrator, m_max_depth, m_rr_depth, m_hide_emitters)
    MI_IMPORT_TYPES(Scene, Sampler, Medium, Emitter, EmitterPtr, BSDF, BSDFPtr)

    AdaTestIntegrator(const Properties &props) : Base(props) {
        std::string as_type_str = props.string("as_type", "empty");
        ASFlags as_type(ASFlags::Empty);
        if (as_type_str == "Array")
            as_type = ASFlags::Array;
        else if (as_type_str == "Quadtree")
            as_type = ASFlags::Quadtree;

        //TODO
        // AS의 다른 parameter도 받아서 생성하자
        AS adaptive_sampler;
        switch (as_type) {
            case ASFlags::Array: {
                adaptive_sampler = LinearAS();
                break;
            }
            case ASFlags::Quadtree: {
                adaptive_sampler = QTAS();
                break;
            }
            default:
                adaptive_sampler = AS();
                //TODO
                // AS는 abstract로 바꾸고 대신 LinearAS의 default 값을 할당하자
        }
    }
    ...
    std::string to_string() const override {
        return tfm::format("AdaTestIntegrator[\n"
                           "  max_depth = %u,\n"
                           "  rr_depth = %u\n"
...
MI_IMPLEMENT_CLASS_VARIANT(AdaTestIntegrator, MonteCarloIntegrator)
MI_EXPORT_PLUGIN(AdaTestIntegrator, "Adaptive Path Tracer integrator (test)");
